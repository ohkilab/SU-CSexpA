...
スケジュールおよび会場（ZoomのURL等）は[今年度の講義について](../schedule)を参照してください．
...
**`\\fs.inf.in.shizuoka.ac.jp\share\submit\(実施年数)情報科学実験A\第3部発表会\中間`**

ファイル名：(実験名)-(部)-(部屋&班).pptx　　　例）CS実験A-Part3-A7.pptx
...
**第三部の場合**
**`\\fs.inf.in.shizuoka.ac.jp\share\submit\(実施年数)情報科学実験A\第3部発表会\最終`**

- ファイル名：(実験名)-(部)-(部屋&班)**\-最終版**.pptx　　例）CS実験A-Part2-A7 **\-最終版**.pptx
