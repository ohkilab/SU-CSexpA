...
```c
// ポート番号を引数として、ソケットを生成し、接続待ち状態にする
// 返り値は接続待ちにしたソケットのファイルディスクリプタ
int exp1_tcp_listen(const char* port) {

  // ソケットアドレス集合であるaddrinfo構造体hintsの宣言・初期化
  // 詳細はMan page of GETADDRINFOを参照
  struct addrinfo hints;
  memset(&hints, 0, sizeof(hints));

  // ソケットアドレスオプション指定
  // 詳細はMan page of GETADDRINFO, Man page of SOCKETを参照
  ...
  // portから実際に使用するaddrinfo構造体resへ値を格納する
  // hintsはresに値を格納する際の基準指定用
  if ((errcode = getaddrinfo(NULL, port, &hints, &res)) != 0) {
    fprintf(stderr, "getaddrinfo():%s\n", gai_strerror(errcode));
    return (-1);
  }

  // 取得したres内情報からソケットを生成,ソケットのファイルディスクリプタ―を取得する
  int sock = 0;
  ...
  // ソケットのファイルディスクリプタ―からソケットに関連するオプションの操作を行う
  // ここではソケットのアドレス再利用を可能にしている
  int socket_option = 1;  // 操作するオプションのプロトコル層
  socklen_t socket_option_size = sizeof(socket_option);
  if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &socket_option, socket_option_size) == -1) {
    perror("setsockopt");
    close(sock);
    freeaddrinfo(res);
    return (-1);
...
```c
// ポート番号・ホスト名からソケットを生成、接続を行う
// 返り値は接続したソケットのファイルディスクリプタ―
...
```makefile
 ...
 C++ = g++
 LD = g++
 CFLAGS = -c -Wall -pedantic-errors -O0 -g3 -std=gnu99 -I../
 LDFLAGS =
