...
Makefileを利用して，以下のようにmakeコマンドでソースコードをビルドします．
...
指定するファイル名については適時変更しながら使用してください．
...
int main(int argc, char** argv) {
  ...
  fd = open(argv[1], O_RDONLY); // O_RDONLYについてはMan page of openを参照
...
make時に警告が出るかもしれませんが、無視してかまいません。
...
int main(int argc, char** argv) {
  ...
  fwrite(buf, sizeof(char), ret, stdout); // stdoutはMan page of STDINを参照
...
int main(int argc, char** argv) {
  ...
  // whileで繰り返し処理することでバッファサイズよりも大きなデータを扱うことができる
  while (ret > 0) {
...
上記の例ではバッファサイズよりも大きなデータを扱うことができることを分かりやすくするために，バッファサイズを4にしています．
